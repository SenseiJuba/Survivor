...
public abstract class AbstractUltimateThingLauncher extends AbstractUltimateWeapon
{
	...
	public void explode(Player p, Location l)
	{
		for(Player ent : l.getWorld().getPlayers())
		{
			...
			{
				double m = 1 / (((double)Utils.pointsBetween(l, ent.getEyeLocation(), null).size()) / 10  / 2 + 0.5) / 2;
				
				GameManager.damageTF(ent, p, (rayonExplosion-ent.getEyeLocation().distance(l))/rayonExplosion*damageExplosion*m, Utils.explosionVector(ent, l, rayonExplosion).multiply(m*valueKB));
			}
		}
