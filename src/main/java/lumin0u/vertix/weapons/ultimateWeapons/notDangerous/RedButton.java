...
public class RedButton extends AbstractNotDangerousUltimateWeapon
{
	...
	public void effect(Player p)
	{
		...
		p.getInventory().addItem(new Mine().getItem(1));

		PlayerManager.getInstance().getTFPlayer(p).setInvicible(true);
		...
		new BukkitRunnable()
		{
			...
			public void run()
			{
				...
				if(i == 100)
				{
					PlayerManager.getInstance().getTFPlayer(p).setInvicible(false);
					this.cancel();
				}
