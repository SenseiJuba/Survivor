	...
	public void decharge(Player p)
	{
		...
		for(Player pl : p.getWorld().getPlayers())
		{
			if(!gm.sameTeam(p, pl))
			{
				...
				TFPlayer tfpl = PlayerManager.getInstance().getTFPlayer(pl);
				
				if(tfpl.getC4Location() != null)
					p.getWorld().strikeLightningEffect(tfpl.getC4Location());
				
				if(tfpl.getTurretLocation() != null)
					p.getWorld().strikeLightningEffect(tfpl.getTurretLocation());
				
				if(tfpl.getTrampoLocation() != null)
					p.getWorld().strikeLightningEffect(tfpl.getTrampoLocation());
				
				if(tfpl.getMineLocations() != null && !tfpl.getMineLocations().isEmpty())
					for(Location l : tfpl.getMineLocations())
