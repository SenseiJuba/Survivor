...
public abstract class AbstractGun extends AbstractWeapon
{
	...
	public void shoot(Player p)
	{
		...
		if(this instanceof LaTornade)
		{
			...
			double j = PlayerManager.getInstance().getTFPlayer(p).getHeavyBulletNb();
			double multi = 0.15;
			...
			TFPlayer tfp = PlayerManager.getInstance().getTFPlayer(p);
		...
		try
		{
			if(PlayerManager.getInstance().getTFPlayer(p).getpower() != null && PlayerManager.getInstance().getTFPlayer(p).getpower().equals(SuperPower.AIMBOT) && !(this instanceof LaTornade))
				precision = getClass().newInstance().precision * 0.4;

			else if(this instanceof LaTornade && PlayerManager.getInstance().getTFPlayer(p).isLookingHeavy())
		...
		counter: for(int counter = 0; counter < range * 4; counter++)
		{
			...
			if(PlayerManager.getInstance().getTFPlayer(p).getpower() != null && PlayerManager.getInstance().getTFPlayer(p).getpower().equals(SuperPower.DMG) && counter % 4 == 1)
			...
			for(Player ent : p.getWorld().getPlayers())
			{
				...
				if(PlayerManager.bodyCub(ent).multiply(hitboxMultiplier).hasInside(point) || PlayerManager.headCub(ent).multiply(hitboxMultiplier).hasInside(point))
				{
					...
					else if(PlayerManager.getInstance().getTFPlayer(p).isLooking() && this instanceof Sniper)// looking sniper
					{
