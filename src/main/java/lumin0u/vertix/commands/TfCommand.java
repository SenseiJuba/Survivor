...
//					p.sendMessage("§aExecuted " + c.getName());
		...
		else if(args[0].equalsIgnoreCase("kill"))
		{
			...
			{
				PlayerManager.getInstance().getTFPlayer(Bukkit.getPlayer(args[1])).addDamager(p);
		...
		{
			p.getInventory().setItem(6, PlayerManager.getInstance().getTFPlayer(p).getKit().getSpecial().getItem(1));
	...
	public void changeCheat(Player p)
	{
		if(cheaters.contains(p))
		{
			...
			p.setHealth(PlayerManager.getInstance().getTFPlayer(p).getKit().getMaxHealth());
		}
		...
		else
		{
			...
			p.setMaxHealth(PlayerManager.getInstance().getTFPlayer(p).getKit().getMaxHealth() * 5);
			p.setHealth(PlayerManager.getInstance().getTFPlayer(p).getKit().getMaxHealth() * 5);
		}
	}
}
