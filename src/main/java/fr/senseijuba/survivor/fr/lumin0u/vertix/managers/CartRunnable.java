...
public class CartRunnable extends BukkitRunnable
{
	...
	public void run()
	{
		...
		for(Player p : cart.getWorld().getPlayers())
		{
			if(p.getLocation().distance(cart.getLocation()) < 4 && gm.getTeamOf(p).equals(t))
			{
				valeur += PlayerManager.getInstance().kitOf(p).getValeurCart() * (PlayerManager.getInstance().getSP(p) != null && PlayerManager.getInstance().getSP(p).equals(SuperPower.PUSHER) ? 2 : 1);
			}
			
			else if(p.getLocation().distance(cart.getLocation()) < 4 && PlayerManager.getInstance().getSP(p) != null && PlayerManager.getInstance().getSP(p).equals(SuperPower.DEFENDER) && !gm.getTeamOf(p).equals(t))
				valeur -= 0.5;
		}
...
//			TF.debug(valeur);
			...
			if(!lastPos.equals(cart.getLocation().getBlock().getLocation()))
			{
				...
				{
					if(cm != null && PlayerManager.getInstance().getTeamCartDirection(p) != null && cm.getCart(PlayerManager.getInstance().getTeamCartDirection(p)) != null && cm.getCart(PlayerManager.getInstance().getTeamCartDirection(p)).getLocation() != null)
