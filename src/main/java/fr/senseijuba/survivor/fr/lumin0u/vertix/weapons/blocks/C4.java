...
public class C4 extends AbstractBlock
{
	public C4()
	{
		super("C4", Material.STONE_BUTTON, 1, 23.9, 10, false, "guns.charge", 5, (Detonateur)Detonateur.getInstance(), Material.STONE_BUTTON, "§6Poser une C4 vous donnera un détonateur");
	}
		...
		try
		{
			for(Player ent : l.getWorld().getPlayers())
			{
				if(ent.getLocation().distance(l) <= 4)
				{
					...
					GameManager.damageTF(ent, p, (4 - ent.getEyeLocation().distance(l)) / 4 * 18 * m, Utils.explosionVector(ent, l, 5).multiply(m * 0.5));
				}
			}
		}catch(Exception e)
		{
