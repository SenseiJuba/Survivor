...
public class Scavenger extends AbstractUltimateGun
{
	public Scavenger()
	{
		super("Scavenger", Material.SULPHUR, 2, 10, 0, 100, 0.00001, false, "guns.scavenger", 30, 0, "�6Dur�e du poison : �a6s", "�6Range : �a100");
	}
	...
	public void shoot(Player p)
	{
		...
		for(int counter = 0; counter < 200; counter++)
		{
			...
			{
				if(ent instanceof Player && GameManager.getInstance(ent.getWorld()).sameTeam(p, ((Player)ent)))
					continue;
				
				if(!(ent instanceof ArmorStand) && ent.getLocation().distance(point) < 2 && ent instanceof LivingEntity && !ent.isDead() && !ent.getName().equals(p.getName()))
				{
					...
					((LivingEntity)ent).addPotionEffect(new PotionEffect(new PotionEffectType(PotionEffectType.POISON.getId())
					{
						@Override
					...
					if(ent instanceof Player)
						PlayerManager.getInstance().addDamagerTo((Player)ent, p);
				}
			}
