...
//						Location l = l1.clone().add(x1.clone().multiply(multi * Math.sin((double)j / 20 * Math.PI * 2d))).add(x2.clone().multiply(multi * Math.cos((double)j / 20 * Math.PI * 2d)));
						...
						p.getWorld().spigot().playEffect(l, Effect.COLOURED_DUST, 0, 0, 0, 0, 0, 1, 0, 10);
					}
					...
					{
						if(ent.getLocation().distance(point) < 2.2 && !GameManager.getInstance(ent.getWorld()).sameTeam(p, ent))
						{
							...
							if(!alreadyFired.contains(ent))
							{
								...
								PlayerManager.getInstance().setFireCauseOf(ent, new FireCause(false, p.getUniqueId(), instance));
